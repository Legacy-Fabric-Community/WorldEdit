...
public class FabricPlayer extends AbstractPlayerActor {
    ...
    public void dispatchCUIEvent(CUIEvent event) {
        ...
        CustomPayloadS2CPacket packet = new CustomPayloadS2CPacket(new Identifier(FabricWorldEdit.MOD_ID, FabricWorldEdit.CUI_PLUGIN_CHANNEL).toString(), buffer);
    ...
    public void print(Component component) {
        this.player.sendMessage(Text.Serializer.deserialize(GsonComponentSerializer.INSTANCE.serialize(WorldEditText.format(component, getLocale()))));
    }

    private void sendColorized(String msg, Formatting formatting) {
        for (String part : msg.split("\n")) {
            LiteralText component = new LiteralText(part);
            component.setStyle(component.getStyle().setColor(formatting));
    ...
    public <B extends BlockStateHolder<B>> void sendFakeBlock(BlockVector3 pos, B block) {
        ...
        if (block == null) {
            ...
            buf.writeVarInt(Block.getByBlockState(FabricAdapter.adapt(block.toImmutableState())));
            try {
                packetOut.read(buf);
            } catch (IOException e) {
                return;
